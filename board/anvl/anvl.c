...
int board_init(void)
{
	u32 *control_devconf1 = OMAP_CONTROL_DEVCONF1;
	...
	*control_devconf1 |= OMAP_CONTROL_DEVCONF1_MMCSDIO2ADPCLKISEL;
...
int misc_init_r(void)
{
	...
	if (twl4030_i2c_read_u8(TWL4030_CHIP_PM_MASTER, &buf,
				TWL4030_PM_MASTER_CFG_P1_TRANSITION)) {
		printf("Error:TWL4030: failed to read the power register\n");
		printf("Could not configure P1 hardware power transitions\n");
	} else {
		buf = buf & ~(TWL4030_PM_MASTER_P_TRANSITION_STARTON_USB |
			TWL4030_PM_MASTER_P_TRANSITION_STARTON_CHG)
			| TWL4030_PM_MASTER_P_TRANSITION_STARTON_PWRON;

		if (twl4030_i2c_write_u8(TWL4030_CHIP_PM_MASTER, buf,
				TWL4030_PM_MASTER_CFG_P1_TRANSITION)) {
			printf("Error:TWL4030: failed to write the power register\n");
			printf("Could not configure P1 hardware power transitions\n");
		} else {
