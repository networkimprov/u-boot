...
void mx25_uart_init_pins(void)
{
	...
	inpadctl = MX25_PIN_PAD_CTL_HYS
		   ...
		   | MX25_PIN_PAD_CTL_PUE
		   | MX25_PIN_PAD_CTL_100K_PU;
...
void mx25_fec_init_pins(void)
{
	...
	inpadctl_100kpd = MX25_PIN_PAD_CTL_HYS
		   | MX25_PIN_PAD_CTL_PKE
		   | MX25_PIN_PAD_CTL_PUE
		   | MX25_PIN_PAD_CTL_100K_PD;
	inpadctl_22kpu = MX25_PIN_PAD_CTL_HYS
		   | MX25_PIN_PAD_CTL_PKE
		   | MX25_PIN_PAD_CTL_PUE
		   | MX25_PIN_PAD_CTL_22K_PU;
	/*
	 * set up output pins with 100K pull-downs
	 * FIXME: need to revisit this
	 * 	PUE is ignored if PKE is not set
	...
	writel(inpadctl_100kpd, &padctl->pad_fec_tx_clk); // 0x1c0

	/* FEC_RX_DV */
	writel(muxmode0, &muxctl->pad_fec_rx_dv);
	writel(inpadctl_100kpd, &padctl->pad_fec_rx_dv); // 0xc0

	/* FEC_RDATA0 */
	writel(muxmode0, &muxctl->pad_fec_rdata0);
	writel(inpadctl_100kpd, &padctl->pad_fec_rdata0); // 0xc0

	/* FEC_TDATA0 */
	writel(muxmode0, &muxctl->pad_fec_tdata0);
	writel(outpadctl, &padctl->pad_fec_tdata0); // 0x40

	/* FEC_TX_EN */
	writel(muxmode0, &muxctl->pad_fec_tx_en);
	writel(outpadctl, &padctl->pad_fec_tx_en); // 0x40

	/* FEC_MDC */
	writel(muxmode0, &muxctl->pad_fec_mdc);
	writel(outpadctl, &padctl->pad_fec_mdc); // 0x40

	/* FEC_MDIO */
	writel(muxmode0, &muxctl->pad_fec_mdio);
	writel(inpadctl_22kpu, &padctl->pad_fec_mdio); // 0x1f0

	/* FEC_RDATA1 */
	writel(muxmode0, &muxctl->pad_fec_rdata1);
	writel(inpadctl_100kpd, &padctl->pad_fec_rdata1); // 0xc0

	/* FEC_TDATA1 */
	writel(muxmode0, &muxctl->pad_fec_tdata1);
	writel(outpadctl, &padctl->pad_fec_tdata1); // 0x40
